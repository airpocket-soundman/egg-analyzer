...
#define MESUREMENT_CYCLE 2000               //測定頻度 us秒/回  defo 2500
...
void loop() {
  ...
  if (SW1State == LOW && SW1StateOld == HIGH){
    ...
    //測定を指定回数自動で繰り返す。
    int mesure_count = MESUREMENT_NUMBER ;   //測定回数(回)
...
void mesure_current(int num){
  ...
  Serial.print("start mesure cycle");
  Serial.println(num);
  for(int i = 0; i < TOTAL_MESUREMENTS; i++){
//    Serial.printf("cycle num = %d / ",i);
    if (i == 0){
      ...
      old = now;
      elapsedTime = now - old; 
      while(elapsedTime <= MESUREMENT_CYCLE + num * 100){
        now = micros();
        elapsedTime = now - old;
      }
      current = ammeter.getCurrent();
...
void saveData(){
  ...
  for (int i = CUTOFF; i < TOTAL_MESUREMENTS / 2 + CUTOFF; i++){
