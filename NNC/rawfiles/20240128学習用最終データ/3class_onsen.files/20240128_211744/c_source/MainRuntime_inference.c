...
typedef struct {
    void *buffer_pool[2];
    void *param_pool[8];
    rt_buffer_allocate_type_t variable_buffers_allocate_type[14];
    ...
    int v8_shape[2];
    rt_variable_t v9; ///< Affine
    int v9_shape[2];
    rt_variable_t v10; ///< Softmax
    int v10_shape[2];
    rt_variable_t v11; ///< Affine_2
    int v11_shape[2];
    rt_variable_t v12; ///< BatchNormalization
    int v12_shape[2];
    rt_variable_t v13; ///< Softmax_2
    int v13_shape[2];
    ...
    rt_function_t f1; ///< Softmax
    rt_variable_t* f1_inputs[1];
    rt_variable_t* f1_outputs[1];
    softmax_local_context_t *f1_local_context;
...
void* nnablart_mainruntime_allocate_context(void** params)
{
    ...
    // Softmax
    c->f1_local_context = malloc(sizeof(softmax_local_context_t));
    (c->f1).num_of_inputs = 1;
    (c->f1_inputs)[0] = &(c->v9);
    (c->f1).inputs = c->f1_inputs;
    (c->f1).num_of_outputs = 1;
    (c->f1_outputs)[0] = &(c->v10);
    (c->f1).outputs = c->f1_outputs;
    (c->f1).local_context = c->f1_local_context;
    (c->f1_local_context)->axis = 1;
    allocate_softmax_local_context(&(c->f1));
    // Affine
    ...
    (c->f3_local_context)->batch_stat = 0;
...
int nnablart_mainruntime_free_context(void* context)
{
    ...
    free_softmax_local_context(&(c->f1));
